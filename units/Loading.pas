...
type
  TLoading = class
    ...
    public
          class procedure ShowF(const Frm : Tform; const msg : string);
          class procedure ShowL(const Lyt: TLayout; const msg : string);
...
class procedure TLoading.ShowF(const Frm : Tform; const msg: string);
var
...
class procedure TLoading.ShowL(const Lyt: TLayout; const msg: string);
var
        FService: IFMXVirtualKeyboardService;
begin
        // Panel de fundo opaco...
        Fundo := TRectangle.Create(Lyt);
        Fundo.Opacity := 0;
        Fundo.Parent := Lyt;
        Fundo.Visible := true;
        Fundo.Align := TAlignLayout.Contents;
        ...
        // Animacao...
        Animacao := TFloatAnimation.Create(Lyt);
        Animacao.Parent := Arco;
        Animacao.StartValue := 0;
        Animacao.StopValue := 360;
        Animacao.Duration := 0.8;
        ...
        // Label do texto...
        Mensagem := TLabel.Create(Lyt);
        Mensagem.Parent := Layout;
        Mensagem.Align := TAlignLayout.Center;
        Mensagem.Margins.Top := 60;
        Mensagem.Font.Size := 13;
        Mensagem.Height := 70;
        Mensagem.Width := Lyt.Width - 100;
        Mensagem.FontColor := $FFFEFFFF;
        Mensagem.TextSettings.HorzAlign := TTextAlign.Center;
        Mensagem.TextSettings.VertAlign := TTextAlign.Leading;
        Mensagem.StyledSettings := [TStyledSetting.Family, TStyledSetting.Style];
